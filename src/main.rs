...
fn run() -> Result<(), Box<dyn Error>> {
    ...
    match arguments.next().as_deref() {
        ...
        Some("play") => {
            let mode = arguments
                .next()
                .map_or(Ok(ui::PlayMode::default()), |value| value.parse())?;
            reject_extra_argument(arguments.next())?;
            ui::play(mode)?;
        }
        Some("watch") => {
            let path = arguments
                .next()
                .ok_or_else(|| invalid_input("watch requires a JSON replay path"))?;
            reject_extra_argument(arguments.next())?;
            ui::watch(Replay::read_json(path)?)?;
        }
        Some("train") => {
...
fn print_training_progress(started: Instant, event: &TrainingProgress) {
    ...
    match event {
        ...
        TrainingProgress::SelfPlayStarted {
            ...
            "[{elapsed}] self-play started: evaluator={evaluator:?}, {games} games ({planned_restarts} archive restarts from {restart_archive} visited prefixes at {restart_simulations} simulations/move), {workers} workers, {simulations} regular simulations/move, {search_batch_size} leaves/inference, repetition contempt={repetition_contempt:.2}, starter draw value={starter_draw_value:.2}"
        ),
        ...
        TrainingProgress::SelfPlayFinished {
            ...
            eprintln!(
                "[{elapsed}] self-play finished: evaluator={evaluator:?}, {games} games ({restarted_games} archive restarts), {examples} examples, absolute first/second/draw={}/{}/{}, mover starter/non-starter/unclassified={}/{}/{}",
                outcomes.first_wins,
            ...
            if evaluator.is_neural() {
                print_inference_stats(&elapsed, "self-play inference", inference);
            } else {
                eprintln!("[{elapsed}] self-play inference: disabled during rollout bootstrap");
            }
        }
        TrainingProgress::SelfPlayResumed {
            ...
            "[{elapsed}] self-play resumed from {games} persisted games generated by {evaluator:?} ({restarted_games} archive restarts, {examples} examples); generation will not be duplicated"
        ),
...
fn print_generation_report(report: &yokai::GenerationReport) {
    println!(
        ...
        report.restart_archive_prefixes,
        report.restart_ply_min,
        report.restart_ply_max,
        report.mean_restart_ply,
